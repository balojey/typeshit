...
- Export a `render(ctx: CanvasRenderingContext2D, state: GameState)` function
- Set canvas dimensions using `devicePixelRatio`:
  ```js
...
- When lives reach 0, call a `onGameOver()` callback
...
- Implement this by translating the canvas context before drawing: `ctx.translate(shakeX, shakeY)`
- Reset translation after each frame
...
Implement these three modes as separate files:
...
`timeAttack.ts` — 60-second timer. Win condition = timer reaches 0 (player survived). Loss = 0 lives OR timer ends with score below a threshold (set to 0 for now). `scoreMultiplier: 1.5`. Add a countdown timer to the HUD when this mode is active.
...
Install on server: `@fastify/jwt`, `@fastify/cookie`, `bcrypt`, `pg` (node-postgres)
